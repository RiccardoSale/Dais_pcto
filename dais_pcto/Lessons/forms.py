...
class TokenForm(FlaskForm):
    ...
    def validate_token(self, field):
        ...
        # Individuazione della lezione interessata se l'utente ne ha già dichiarato la presenza
        q = db.session.query(User).join(Lesson._users).filter(User._user_id == self.user.data,
                                                              Lesson._lesson_id == self.id.data).first()
        # Se q ha un risultato significa che a quella lezione ha già dichiarato la presenza
        if q:
